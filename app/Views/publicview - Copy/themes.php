                       ...
                       <div class="blog-sidebar">  
                        <div class="volt-sin-sidebar ">
                           <div class="single-news bgdata1 wow fadeInUp" data-wow-delay=".3s">
                                ...
                                <img class="m-w-m-h-modules-themes" src="<?=base_url()?>/master/uploads/modulebanner/<?php echo $getmodules->m_img;?>" alt="">
                                <?php }else{ ?>
                                <img class="m-w-m-h-modules-themes" src="<?=base_url()?>/master/uploads/noimage.png">
                                <?php } ?> 
                                <div class="themes-det">
                                    <h4><?php echo $getmodules->m_name;?></h4>
                                </div>
                            </div>
                        </div> 
                           
                        <?php  if ($modulelistelse) { ?>    
                         <div class="sin-sidebar bradius10 border1 mob-hide">
                           <ul class="cat-post border1 padding15"> 
                            <?php  
                            $i=1; foreach ($modulelistelse as $getModule) {
                            if($getModule['m_url']){
                                $link = $getModule['m_url'];
                            }else{
                            $link =  base_url('subject/'.$geturlsubject->slug.'/class'.$geturlclass->class_id.'/'. $getModule['slug']); 
                            }       
                            ?>
                            <li><a href="<?=$link?>" class="fsize-18"><?=$getModule['m_name']?> </a></li>
                    ...
                    <div class="row">
                       <?php  if (is_array($themeList) && count($themeList) > 0)  { 
                        ...
                        <div class="col-md-4 col-md-6 col-xl-3">
                            <a href="<?=base_url()?>/subject/<?php echo $geturlsubject->slug;?>/class<?php echo $geturlclass->class_id; ?>/<?php echo $getmodules->slug.'/'.$getthemeData['slug'];?>"> 
                            <div class="single-news themes bgdata2 wow fadeInUp" data-wow-delay=".3s">
